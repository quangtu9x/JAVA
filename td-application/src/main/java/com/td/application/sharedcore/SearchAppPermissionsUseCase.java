...
public class SearchAppPermissionsUseCase {

    private final AppPermissionRepository appPermissionRepository;

    public PaginationResponse<AppPermissionDto> execute(SearchAppPermissionsRequest request) {
        try {
            Pageable pageable = buildPageable(request);
            var page = appPermissionRepository.search(request, pageable);

            List<AppPermissionDto> items = page.getContent().stream()
                .map(AppPermissionDtoMapper::map)
                .toList();
    ...
    private Pageable buildPageable(SearchAppPermissionsRequest req) {
        String sortBy = (req.getSortBy() == null || req.getSortBy().isBlank())
            ? "moduleKey" : req.getSortBy();
